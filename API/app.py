...
app = Flask(__name__)
IMGFOLDER = '\\\gush.reshet.local\Homedirs\e318243755\\Android\\Uploads'
...
class LoginForm(FlaskForm):
    username = StringField(u'שם משתמש', validators=[InputRequired(), Length(min=3, max=20)])
...
class RegisterForm(FlaskForm):
    ...
    username = StringField('name', validators=[InputRequired(), Length(min=4, max=20)])
...
def updatePgisha():
    if request.method=='POST':
        ...
        now = returnCurrentHour()
        sendPgisha(username,title,time,location,eishKesher,latcor,longcor,sogEsek,sogAnaf,sogTafkid,sogTozaa,name)
